...
const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:`You are an AfDB ROE portfolio intelligence analyst. Data:\n${ctx}\nBe concise, use numbers, format with line breaks.`,messages:[{role:"user",content:aiQ}]})});
const data=await res.json();setAiR(data.content?.[0]?.text||"No response received.");}catch(e){setAiR(`Error: ${e.message}\n\nWhen deployed standalone, add ANTHROPIC_API_KEY as environment variable.`);}setAiL(false);};
...
<div style={{marginTop:8,fontSize:10,color:"#bbb"}}>Uses Claude Sonnet via Anthropic API. In Claude.ai artifacts: works automatically. Standalone deployment: add ANTHROPIC_API_KEY as env variable.</div>
</div>}</div></div>}
